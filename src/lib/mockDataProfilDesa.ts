/**
 * Mock Data untuk Profil Desa
 * Development lokal tanpa Firebase
 */

import { ProfilDesa } from "./profilDesaTypes";
import { DEFAULT_PROFIL_DESA } from "./profilDesaService";

// LocalStorage Key
const STORAGE_KEY = 'mock_profil_desa';

// Helper
const isBrowser = typeof window !== 'undefined';

const loadFromStorage = (defaultValue: ProfilDesa): ProfilDesa => {
  if (!isBrowser) return defaultValue;
  try {
    const stored = localStorage.getItem(STORAGE_KEY);
    if (stored) {
      return JSON.parse(stored) as ProfilDesa;
    }
  } catch (error) {
    console.error("Error loading profil desa from localStorage:", error);
  }
  return defaultValue;
};

const saveToStorage = (data: ProfilDesa): void => {
  if (!isBrowser) return;
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
  } catch (error) {
    console.error("Error saving profil desa to localStorage:", error);
  }
};

// Default profil desa untuk mock
const DEFAULT_MOCK_PROFIL_DESA: ProfilDesa = {
...
export let mockProfilDesa: ProfilDesa = loadFromStorage(DEFAULT_MOCK_PROFIL_DESA);

// Update profil desa
export const mockUpdateProfilDesa = (data: Partial<ProfilDesa>): ProfilDesa => {
  mockProfilDesa = { ...mockProfilDesa, ...data };
  saveToStorage(mockProfilDesa);
  return mockProfilDesa;
};

// Reset ke default
export const resetMockProfilDesa = (): ProfilDesa => {
  mockProfilDesa = DEFAULT_MOCK_PROFIL_DESA;
  saveToStorage(mockProfilDesa);
  return mockProfilDesa;
};
