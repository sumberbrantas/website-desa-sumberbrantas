...
import { useAccommodationActions } from "@/hooks/useAccommodations";
...
const CreateAccommodationPage = () => {
  ...
  const { create, loading, error } = useAccommodationActions();
  ...
  const handleSave = async (data: {
    ...
    if (!user || !profile) {
      alert("Anda harus login untuk menambahkan penginapan.");
      return;
    }
    ...
    if (!data.image) {
      alert("Gambar penginapan wajib diupload.");
      return;
    }

    if (storageError) {
      alert("Tidak dapat menyimpan penginapan karena storage penuh. Silakan kosongkan storage terlebih dahulu.");
      return;
    }
    ...
    if (result) {
      setSuccess("Penginapan berhasil ditambahkan!");
      setTimeout(() => {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <PageHeader title="Tambah Penginapan" subtitle="Tambahkan tempat penginapan baru di website" actions={headerActions} mounted={mounted} />
