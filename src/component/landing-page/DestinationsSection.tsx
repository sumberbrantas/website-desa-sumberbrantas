    ...
    <section className="py-16 px-4" style={{ backgroundColor: "var(--background)" }}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-earth-dark">Destinasi Wisata</h2>
          <p className="text-lg max-w-2xl mx-auto text-earth-muted">Temukan tempat wisata menarik di desa kami</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {destinations.map((dest) => (
            <div key={dest.id} className="group card-earth overflow-hidden">
              ...
              <div className="p-5">
                <h3 className="font-bold text-xl mb-2 text-earth-dark">{dest.name}</h3>
                {dest.location && (
                ...
                {dest.description && <p className="text-sm text-earth-muted line-clamp-2 mb-4">{dest.description}</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center mt-10">
          <Link href="/destinations" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition-all duration-300 hover:scale-105 active:scale-95" style={{ backgroundColor: "var(--primary)", color: "white" }}>
