    ...
    <section className="py-16 px-4" style={{ backgroundColor: "var(--background)" }}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8">
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-earth-dark">Produk UMKM</h2>
          <p className="text-lg max-w-2xl mx-auto mb-6 text-earth-muted">Produk lokal berkualitas dari pelaku usaha desa</p>

          <div className="flex flex-wrap justify-center gap-2">
            <button
              onClick={() => setSelectedCategory("all")}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedCategory === "all" ? "text-white shadow-md" : "bg-white text-earth-primary hover:bg-earth-sand"}`}
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedCategory === cat ? "text-white shadow-md" : "bg-white text-earth-primary hover:bg-earth-sand"}`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.slice(0, 8).map((product) => {
            ...
            return (
              <div key={product.id} className="group card-earth overflow-hidden">
                ...
                <div className="p-4">
                  <h3 className="font-bold text-lg mb-1 line-clamp-1 text-earth-dark">{product.name}</h3>
                  <p className="text-sm text-earth-muted line-clamp-2 mb-3 min-h-[2.5rem]">{product.description}</p>
                  <p className="font-bold text-xl mb-4 text-earth-dark">{formatProductPrice(product.price)}</p>
                  ...
                  {waLink ? (
                    ...
                    <div className="w-full py-2.5 rounded-lg font-medium bg-gray-100 text-gray-400 text-center cursor-not-allowed">WhatsApp tidak tersedia</div>
                  )}
                </div>
              </div>
            );
        ...
        <div className="text-center mt-10">
          <Link href="/products" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition-all hover:scale-105 active:scale-95" style={{ backgroundColor: "var(--primary)", color: "white" }}>
